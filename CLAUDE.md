...
- **`uvx`/`uv` are not preinstalled** in this sandbox, so any target that shells out to them (e.g. the generated infra project's `checkov` target, or `ruff`-based Python lint tests) fails with `uvx: not found` / `spawnSync uvx ENOENT` — install with `curl -LsSf https://astral.sh/uv/install.sh | sh` (installs to `~/.local/bin`) before running such targets. `~/.local/bin` isn't on `PATH` for every tool invocation in this harness (each Bash call may start a fresh non-login shell) — symlink `uv`/`uvx` into `/usr/local/bin` (already on `PATH`) rather than relying on `~/.bashrc`/`~/.profile` having been sourced.
- **Creating a fresh workspace with the locally-built plugin needs a local npm registry, not `pnpm link`** — `@aws/create-nx-workspace` always resolves the preset as `@aws/nx-plugin@<its own package.json version>`, which in an unpublished dev checkout is the monorepo's placeholder `0.0.0`. A real (ancient, unrelated) `@aws/nx-plugin@0.0.0` is already published on the public npm registry, so a plain `pnpm create @aws/nx-workspace` silently installs *that* instead of your local build (symptom: the created workspace's `generators.json` lists only a handful of very old generators like `trpc#backend`/`cloudscape-website#app` instead of the current ~70). `pnpm link` doesn't help here because workspace creation shells out to a separate `npm install` in a directory that doesn't exist yet when you'd run `link`. Fix: run a local Verdaccio (config already checked in at `.verdaccio/config.yml`) and publish the three packages to it before creating the workspace: `npx -y verdaccio@6.9.2 --config .verdaccio/config.yml --listen 4873 &`, then from each of `dist/packages/{nx-plugin,nx-plugin-mcp,create-nx-workspace}` run `npm publish --registry http://localhost:4873/` (add `--tag e2e` for `nx-plugin-mcp`, per `e2e/src/global-setup.ts`), then point the `@aws` scope at it — `npm config set @aws:registry http://localhost:4873/ && npm config set //localhost:4873/:_authToken secretVerdaccioToken` (writes to `~/.npmrc`, which `@aws/create-nx-workspace`'s own `npm install` step reads) — before running `node dist/packages/create-nx-workspace/bin/index.cjs <name> --no-interactive`. Re-publishing after a code change requires `npm unpublish @aws/nx-plugin@0.0.0 --registry http://localhost:4873/ --force` first (Verdaccio rejects re-publishing the same version); for a fast iteration loop on an already-created workspace, skip the registry round-trip and just overwrite `node_modules/@aws/nx-plugin` with the freshly built `dist/packages/nx-plugin` directly.
- **`@aws/create-nx-workspace`'s own `npm install` step can fail with `TypeError: Cannot read properties of null (reading 'edgesOut')`** from `@npmcli/arborist`'s peer-dependency resolution (`npm error ... at #loadPeerSet ... at Arborist.buildIdealTree`) on this sandbox's default npm 10.9.8, for the base preset's own dependency set (vite/vitest/lightningcss/rolldown peer graph) — unrelated to any generator or connection under test. Fixed by upgrading to npm 11 first: `npm install -g npm@11`.
